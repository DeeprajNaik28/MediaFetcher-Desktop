...
def download_task(data):
    def hook(d):
        ...
        if d['status'] == 'downloading':
            ...
            progress_data["eta"] = d.get('_eta_str', '0 sec')
...
def download():
    ...
    threading.Thread(target=download_task, args=(data,)).start()
...
def get_info():
    ...
    if "entries" in info:
        ...
        return jsonify({"type": "playlist", "videos": videos})

    # 🔥 SINGLE VIDEO
    formats = []
    for f in info["formats"]:
        size = f.get("filesize") or f.get("filesize_approx")
        size = round(size / (1024 * 1024), 2) if size else "N/A"
